...
class ChatScreen extends StatefulWidget {
  const ChatScreen({Key? key,required this.isProvider,required this.CId,required this.PId,required this.orderId}) : super(key: key);
...
class _ChatScreenState extends State<ChatScreen> {
      ...
      if(images!.length==1){
       ...
       url = await ref.getDownloadURL();
          await FirebaseFirestore.instance
              .collection('Chats')
              .doc(widget.orderId)
              .collection(widget.orderId.toString())
              .doc()
              .set({
            'provider':
            widget.isProvider?'p':'null',
            'customer': !widget.isProvider?'c':'null',
            'time': Timestamp.now(),
            'image_url':url,
  ...
  Future<void> getMassages(List<QueryDocumentSnapshot> allMessages ) async{
      ...
      for(int i = 0 ; i< allMessages.length;i++){
        ...
        if(time.toDate().day==allTime[j].toDate().day &&
         ...
         // allMassages2.add(allMessages[i]);
          if(allMessages[i]['customer']=='null'&&allMessages[i]['provider']=='p'&&allMessages[i]['image_url']!='null'){
            messages.add(
                Message(
          ...
          }else if(allMessages[i]['customer']=='c'&&allMessages[i]['provider']=='null'&&allMessages[i]['image_url']!='null'){
            messages.add(
                Message(
                  message: allMessages[i]['image_url'],
                  image: true,
    ...
    return Scaffold(
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            StreamBuilder(
              stream: FirebaseFirestore.instance.collection('Chats').doc(widget.orderId.toString())
                  .collection(widget.orderId.toString()).snapshots(),
              builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                print(snapshot.data!);
                getMassages(snapshot.data!.docs);
